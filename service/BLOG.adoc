...
image::story-time-ai-response.png[]
...
image::adinn-elevator-music.png[]
...
image::doctor-niephaus.png[]

Ok. But he did share this promising prototype!

image::graalvm-prototype.png[]
...
On my machine, it starts up in ~100ms! Like a rocket! But I don't really care all that much about that because this is a standalone, long-lived service. What I care about is the https://en.wikipedia.org/wiki/RSS[Resident Set Size (RSS)]. Note the process identifier (PID) - it'll be in the logs. If the PID is 5, then get the RSS like this using the `ps` utility on most Unixes:
